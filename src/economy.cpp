			...
			if (v->owner == old_owner && IsCompanyBuildableVehicleType(v->type)) {
				if (new_owner == INVALID_OWNER) {
					if (v->Previous() == nullptr) {
						if (IsInfrastructureSharingEnabled(VehicleType::Train) && v->type == VehicleType::Train && (Train::From(v)->IsFrontEngine() || Train::From(v)->IsFrontWagon()) && !Train::From(v)->IsVirtual()) {
