...
static void NormaliseSubtypes(Train *chain)
{
	...
	if (chain->IsWagon()) {
		if (!chain->IsFrontWagon()) chain->SetFrontWagon();
		has_front = true;
...
static void ReverseTrainNoSwapVehicles(Train *v)
{
	...
	/* SetNext updates the new-next's previous, but not the current vehicle's.
	 * After reversal the new head's previous must be explicitly cleared. */
	new_order.front()->SetPrevious(nullptr);

	/* Chain reversal flips DrivingBackwards so that GetMovingFront() continues
	 * to return the physically-front vehicle (now Last() instead of First()).
	 * NormaliseSubtypes is called after reversal to set primary flags on the
	 * new chain head, which may be an articulated part. */
	for (Train *u : vehicles) {
