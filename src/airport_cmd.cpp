...
/**
 * Area (in tiles) one runway of a custom airport is charged for, as a multiple of the airtype's
 * minimum runway length. A runway also needs taxiways, aprons and a terminal, so it takes up more
 * ground than its own length; this factor keeps the noise of a custom airport in the same range as
 * that of the stock airports using the same air type. Increase it to make custom airports cheaper.
 */
static const uint CUSTOM_AIRPORT_TILES_PER_RUNWAY_FACTOR = 2;
 ...
 */
uint16_t CalculateAirportNoiseLevel(const Airport airport, AirType airtype = INVALID_AIRTYPE)
{
	...
	/* Custom airports are charged for the size of their area: convert it into the number of runways it
	 * can hold (including their supporting tiles), capped at the number of runways the airtype allows. */
	if (airport.type == AT_CUSTOM) {
		const uint tiles_per_runway = CUSTOM_AIRPORT_TILES_PER_RUNWAY_FACTOR * air_type_info->min_runway_length;
		if (tiles_per_runway == 0) return air_type_info->base_noise_level;
		const uint runways = std::min<uint>((airport.num_tiles + tiles_per_runway - 1) / tiles_per_runway, air_type_info->max_num_runways);
		return static_cast<uint16_t>(std::min<uint>(air_type_info->base_noise_level + runways * air_type_info->runway_noise_level, UINT16_MAX));
	}

	return static_cast<uint16_t>(air_type_info->base_noise_level +
 ...
 */
uint16_t GetAirportNoiseLevelForDistance(uint noise_level, uint distance)
{
	...
	return static_cast<uint16_t>(std::min<uint>(noise_reduction >= noise_level ? 1 : noise_level - noise_reduction, UINT16_MAX));
}
...
void UpdateAirportsNoise()
{
	...
	for (const Station *st : Station::Iterate()) {
		if (st->airport.tile != INVALID_TILE && st->airport.type != AT_OILRIG) {
			uint16_t noise_level = CalculateAirportNoiseLevel(st->airport);
			uint dist;
			BitmapTileArea bta;
...
CommandCost AddRunway(DoCommandFlags flags, TileIndex start_tile, TileIndex end_tile, AirType air_type, AirportTileType runway_type)
{
	...
	uint pre_noise;
	/* A custom airport is charged for the size of its area; adding a runway turns this airport into one. */
	const bool custom_airport = st->airport.type == AT_CUSTOM;
	Airport predicted = st->airport;
	predicted.type = AT_CUSTOM;
	uint post_noise = CalculateAirportNoiseLevel(predicted, air_type) - CalculateAirportNoiseLevel(st->airport, air_type);
	CommandCost ret = CheckTownAuthorityForAirports(start_tile, st, BitmapTileArea(), &pre_town, &post_town, pre_noise, post_noise);
	if (!custom_airport && ret.Failed()) return ret;
...
CommandCost CmdChangeAirportTiles(DoCommandFlags flags, TileIndex start_tile, TileIndex end_tile, AirType air_type, AirportTileType air_tile_type, AirportTiles infra_type, DiagDirection direction, bool adding, bool diagonal)
{
	...
	} else if (st->airport.type != AT_CUSTOM) {
		/* Check noise limits: adding structures turns this airport into a custom airport, which is
		 * charged for the size of its area, so check the noise of the resulting airport. */
		Town *pre_town = nullptr;
		Town *post_town = nullptr;
		uint pre_noise;
		Airport predicted = st->airport;
...
CommandCost CmdAddRemoveAirportTiles(DoCommandFlags flags, TileIndex start_tile, TileIndex end_tile, bool adding, AirType at, StationID station_to_join, bool adjacent)
{
	...
	Town *post_town = nullptr;
	uint pre_noise;
	/* This command creates/extends custom airports, so charge the noise of the resulting area size:
	 * predict the state of the airport after the tiles of the area have been added. */
	Airport predicted = (st != nullptr) ? st->airport : Airport();
	predicted.type = AT_CUSTOM;
	if (st == nullptr || !st->facilities.Test(StationFacility::Airport)) {
		predicted.tile = new_location.tile;
		predicted.num_tiles = 0;
	}
	for (TileIndex t : new_location) {
		if (!IsAirportTile(t)) predicted.num_tiles++;
	}
	uint post_noise = CalculateAirportNoiseLevel(predicted, at) - CalculateAirportNoiseLevel((st != nullptr) ? st->airport : Airport(), at);
...
CommandCost CmdBuildAirport(DoCommandFlags flags, TileIndex tile, uint8_t airport_type, uint8_t layout, AirType air_type, DiagDirection rotation, StationID station_to_join, bool allow_adjacent)
{
	...
	uint post_noise;
	if (custom_airport) {
		/* Extending an airport turns it into a custom airport, which is charged for its area size. */
		Airport predicted = st->airport;
		predicted.type = AT_CUSTOM;
		for (TileIndex t : new_airport_tiles) {
			if (!IsAirportTile(t)) predicted.num_tiles++;
		}
		post_noise = CalculateAirportNoiseLevel(predicted, air_type) - CalculateAirportNoiseLevel(st->airport, air_type);
	} else {
		post_noise = as->GetAirportNoise(air_type);
	}
